...
import org.globus.workspace.client_core.actions.Pause_Group;
import org.globus.workspace.client_core.actions.Pause_Instance;
...
public class Pause extends SingleShotMode {

    public Pause(Print print,
    ...
    protected void setInstanceAction() throws ParameterProblem {
        this.instanceAction =
                new Pause_Instance(this.epr,  this.stubConf, this.pr);
    }

    protected void setGroupAction() throws ParameterProblem {
        this.groupAction =
                new Pause_Group(this.epr,  this.stubConf, this.pr);
    }
    ...
    public void runImpl() throws ParameterProblem, ExecutionProblem, ExitNow {
        ...
        if (this.args.dryrun) {
            ...
            if (this.pr.enabled()) {
                ...
                if (this.pr.useThis()) {
                    this.pr.infoln(PrCodes.PAUSE__DRYRUN, msg);
    ...
    private void _runImpl() throws ExecutionProblem,
        ...
        if (this.instanceAction != null) {
            ((Pause_Instance)this.instanceAction).pause();
        } else if (this.groupAction != null) {
            ((Pause_Group)this.groupAction).pause();
        }
    }
