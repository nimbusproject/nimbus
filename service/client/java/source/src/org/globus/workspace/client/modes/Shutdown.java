...
import org.globus.workspace.client_core.actions.Shutdown_Instance;
import org.globus.workspace.client_core.actions.Shutdown_Group;
...
public class Shutdown extends SingleShotMode {

    public Shutdown(Print print,
    ...
    protected void setInstanceAction() throws ParameterProblem {
        this.instanceAction =
                new Shutdown_Instance(this.epr,  this.stubConf, this.pr);
    }

    protected void setGroupAction() throws ParameterProblem {
        this.groupAction =
                new Shutdown_Group(this.epr,  this.stubConf, this.pr);
    }
    ...
    public void runImpl() throws ParameterProblem, ExecutionProblem, ExitNow {
        ...
        if (this.args.dryrun) {
            ...
            if (this.pr.enabled()) {
                ...
                if (this.pr.useThis()) {
                    this.pr.infoln(PrCodes.SHUTDOWN__DRYRUN, msg);
    ...
    private void _runImpl() throws ExecutionProblem,
        ...
        if (this.instanceAction != null) {
            ((Shutdown_Instance)this.instanceAction).shutdown();
        } else if (this.groupAction != null) {
            ((Shutdown_Group)this.groupAction).shutdown();
        }
    }
}
