...
class TestEC2Submit(unittest.TestCase):
    ...
    def test_ec2_idempotent_submit(self):
        ...
        assert res1.instances[0].state == res2.instances[0].dns_name, "diff states"

        # check that no other instance is running
        assert len(ec2conn.get_all_instances()) == 1, "!1 instances running"
        ...
        assert len(ec2conn.get_all_instances()) == 2, "!2 instances running"
        ...
        assert len(ec2conn.terminate_instances([res1.instances[0].id])) == 1
        ...
        assert res4.state == 'terminated', "state was %s" % res4.state
